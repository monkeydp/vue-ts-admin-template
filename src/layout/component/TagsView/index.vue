<template>
  ...
  >
    ...
    >
      ...
      >
        {{ $translate('route.' + tag.meta.title) }}
        <span
            v-if="!isAffix(tag)"
            class="el-icon-close"
    ...
    >
      ...
      >
        {{
          $translate('tagsView.close')
        }}
      </li>
      <li @click="closeOthersTags">
        {{ $translate('tagsView.closeOthers') }}
      </li>
      <li @click="closeAllTags(selectedTag)">
        {{ $translate('tagsView.closeAll') }}
      </li>
    </ul>
  </div>
</template>
...
import {TagsViewModule, TagView} from '@/store/module/TagsView'
  ...
  private moveToCurrentTag() {
    ...
    this.$nextTick(() => {
      for (const tag of tags) {
        if ((tag.to as TagView).path === this.$route.path) {
          (this.$refs.scrollPane as ScrollPane).moveToTarget(tag as any)
          // When query is different then update
          if ((tag.to as TagView).fullPath !== this.$route.fullPath) {
